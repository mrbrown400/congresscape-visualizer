...
## Overview
Congresscape Visualizer delivers a real-time, personalized feed of U.S. government activity spanning the Legislative, Judicial, and Executive branches. The platform consists of a modular FastAPI backend with PostgreSQL + pgvector storage, an ingestion pipeline that normalizes diverse data sources, and a React Native mobile client that renders a TikTok-style news feed.
...
- **React Native App**: Presents updates as vertically scrolling cards, with tabs, filters, and personalization controls. Built to share UI modules with a future web client.
...
5. **Serve**: FastAPI endpoints expose sorted feeds, personalized filters, search, and saved items; WebSocket/SSE ready for real-time push.
6. **Present**: React Native app consumes feed endpoints, displaying cards with color-coded branch badges and interactions (save/share/comment/notify).
...
- Mobile app uses feature-based structure (`features/feed`, `features/onboarding`, etc.) to scale to web and desktop clients.
